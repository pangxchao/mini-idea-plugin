...
public class CreationsAction extends AnAction implements EventListener {
    ...
    private static void generator(List<DbTable> tableList, VirtualFile file) {
        ...
        try (OutputStreamWriter writer = new FileWriter(outFile)) {
            ...
            tableList.stream().filter(Objects::nonNull).sorted(Comparator.comparing(DasObject::getName)).forEach(table -> {
                ...
                try {
                    if (!StringUtils.equals(name, database.get(PREV_DB_NAME))) {
                    ...
                    for (DasColumn column : DasUtil.getColumns(table)) {
                        ...
                        String columnType = getDataTypeAndLength(column.getDataType());
                        writer.write(format("\t%s %s %s", column.getName(), columnType, column.isNotNull() ? "NOT NULL" : ""));
                        // 字段默认值
                        if (StringUtils.isNotEmpty(column.getDefault())) {
    ...
    private static String getDataTypeAndLength(@NotNull DataType dataType) {
        if (StringUtils.isEmpty(dataType.typeName)) {
            return "";
        }
